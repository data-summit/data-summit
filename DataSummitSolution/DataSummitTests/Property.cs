...
namespace DataSummitTests
{
    ...
    public class PropertyTests
    {
        ...
        public void Create_new_property()
        {
            ...
            var mockPropertiesDbSet = new Mock<DbSet<Properties>>();
            ...
            var mockProperties = new Property(mockContext.Object);

            mockProperties.CreateProperty(property);

            mockPropertiesDbSet.Verify(m => m.Add(It.IsAny<Properties>()), Times.Once());
        ...
        {
            var testProperties = new List<Properties>
            {
                new Properties
                {
                    PropertyId = 1,
            ...
            var mockPropertyDbSet = new Mock<DbSet<Properties>>();
            mockPropertyDbSet.As<IQueryable<Properties>>().Setup(m => m.Provider).Returns(testProperties.Provider);
            mockPropertyDbSet.As<IQueryable<Properties>>().Setup(m => m.Expression).Returns(testProperties.Expression);
            mockPropertyDbSet.As<IQueryable<Properties>>().Setup(m => m.ElementType).Returns(testProperties.ElementType);
            mockPropertyDbSet.As<IQueryable<Properties>>().Setup(m => m.GetEnumerator()).Returns(testProperties.GetEnumerator());
            ...
            var mockPropertyService = new Property(mockContext.Object);
